...
<div class="app-container">
  <div class="sidebar">
    ...
    <div class="account-info">
      ...
      <div class="account-info-name">Sanchaita B.</div>
  ...
  <div class="app-content">
    <div class="app-content-header">
      <h1 class="app-content-headerText">Users Lists</h1>
      <button class="app-content-headerButton">Add Users</button>
    </div>
    ...
    <table>
        ...
        <tbody>
            <?php foreach ($admin as $index => $product): ?>
                <tr>
                    <td><?= $index + 1 ?></td>
                    <td><?= $product['name'] ?></td>
                    <td><?= $product['email'] ?></td>
                    <td><?= $product['membership_duration'] ?></td>
                    <td><?= $product['start_date'] ?></td>
                    <td><?= $product['expiration_date'] ?></td>
                    <td>
                       <button class="reminder-button" onclick="sendReminder('<?=($product['email']) ?>')">Reminder Send</button>
                    </td>
                </tr>
            <?php endforeach; ?>
        </tbody>
...
<script>
    ...
    const encodedEmail = encodeURIComponent(email); // Encode email for URL safety
    console.log("Encoded email:", encodedEmail); // Log encoded email

    if (confirm("Are you sure you want to send a reminder?")) {
        fetch(`/sendReminder/${encodedEmail}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
                'Accept': 'application/json',
                'X-CSRF-TOKEN': '<?= csrf_hash() ?>' // Include CSRF token here
            },
            body: JSON.stringify({ email: email }) // Optional: Send email in request body too
        })
        .then(response => {
            if (!response.ok) {
